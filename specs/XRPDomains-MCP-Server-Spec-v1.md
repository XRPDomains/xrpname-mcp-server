...
- For each domain → GET `/api/xrplnft/getAddress?domain=<domain>`
- Cache responses 60s per domain
- Parallelise up to 10 concurrent requests
...
Backend mapping: GET `/api/xrplnft/getBithompNFT?address=...` (existing endpoint with address param — verify with backend team) + GET `/api/xrplnft/getName?address=...` for primary.
...
{
  address: string;
  incoming: Array<{ domain, nftoken_id, offer_id, sender, expiration, amount_drops }>;
  outgoing: Array<{ domain, nftoken_id, offer_id, destination, expiration, amount_drops }>;
  counts: { incoming: number; outgoing: number; total: number };
}
```

Backend mapping: GET `/api/xrplnft/getOfferByDestination?address=...` + GET `/api/xrplnft/getOfferByOwner?address=...`. Backend team should ship a unified `/api/xrplnft/getPending` (see API audit spec); until then, run both in parallel.
...
Backend mapping: GET `/api/xrplnft/getAddress?domain=...` (the response shape we verified in the API audit). Cache 60s per domain.
...
### 10.1 Existing endpoints used

| Endpoint | Used by tool | Cache TTL |
|---|---|---|
| `GET /api/xrplnft/getAddress?domain=...` | check_domains, get_domain_profile | 60s |
| `GET /api/xrplnft/getName?address=...` | get_portfolio, check_domains (owner reverse) | 60s |
| `GET /api/xrplnft/getBithompNFT?nftid=...` | get_domain_profile (history if needed) | 60s |
| `GET /api/xrplnft/getOfferByOwner?address=...` | get_pending_offers | 10s |
| `GET /api/xrplnft/getOfferByDestination?address=...` | get_pending_offers | 10s |
...
### 10.2 New endpoint needed (one)

`POST /api/refcode/validateCode?code=ABC` — quick existence check for refcode validity before embedding it in a transaction. Phase 1 can skip this — agent passes refcode through, backend rejects at createOrder time if invalid.
...
|---|---|---|
| `mcp:getAddress:<domain>` | E1 response | 60s |
| `mcp:getName:<address>` | E2 response | 60s |
| `mcp:portfolio:<address>` | E10 response | 30s |
| `mcp:pending:<address>` | E8 + E9 merged | 10s |

Cache invalidation on tx success — when `send_signed_tx` returns success, invalidate any cache key that may have changed (e.g. portfolio of buyer + seller, pending of both).
