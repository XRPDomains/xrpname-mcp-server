#!/usr/bin/env node
/**
 * x402 buy client (AGENT side) — dry-run / live test for /mcp/x402/register.
 *
 * This runs on the PAYER's machine, NOT the XRPName server. It signs the XRPL
 * Payment (via x402Fetch) and then signs the NFTokenAcceptOffer to take custody.
 *
 * Install (in this folder or anywhere):
 *   npm install x402-xrpl xrpl dotenv
 *
 * .env (this client's own file — the SEED lives here, never on the server):
 *   XRPL_BUYER_SEED=sEd...                 # payer wallet seed (mainnet has real XRP!)
 *   RESOURCE_URL=https://xrpdomains.xyz/mcp/x402/register
 *   DOMAIN=myx402test.xrp                  # ROOT domain (1 label + tld), NOT a subname
 *   XRPL_NETWORK=xrpl:0                     # xrpl:0 mainnet · xrpl:1 testnet
 *   XRPL_RPC=wss://xrplcluster.com          # mainnet node (testnet: wss://s.altnet.rippletest.net:51233)
 *
 * Run:  node scripts/x402-buy-client.mjs
 */
...
// Domain from CLI arg (e.g. `node x402-buy-client.mjs test.myname.xrp`) or DOMAIN env.
